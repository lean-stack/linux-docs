...
Globs sind beim Löschen besonders praktisch – aber auch besonders gefährlich:

```bash
rm *.log          # alle Log-Dateien löschen
rm tmp-*          # alle Dateien, die mit "tmp-" beginnen
rm *.jpg *.png    # alle Bilder löschen
```
...
<Aside type="tip">
Bei Globs empfiehlt es sich, das Muster zuerst mit `echo` oder `ls` zu prüfen – so sieht man genau, welche Dateien betroffen wären, bevor man löscht:

```bash
echo *.log        # zeigt, was der Glob trifft
ls *.log          # zeigt es mit Details
rm *.log          # erst dann löschen
```
</Aside>
